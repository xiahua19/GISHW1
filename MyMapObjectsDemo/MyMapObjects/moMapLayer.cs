...
namespace MyMapObjects
{
    ...
    public class moMapLayer
    {
        ...
        private moFeatures _SelectedFeatures = new moFeatures();// 选中的要素集合
        private moRectangle _Extent = new moRectangle(double.MaxValue, double.MinValue,
            double.MaxValue, double.MinValue);// 图层范围
        ...
        public moMapLayer(string name, moGeometryTypeConstant shapeType,
            moFields attributesFields, string path)
        {
            _Name = name;
            _ShapeType = shapeType;
            _AttributeFields = attributesFields;
            ...
            double tolerance)
        {
            ...
            {
                // 按点选
                sSelection = SearchFeaturesByPoint(new moPoint(selectingBox.MinX, selectingBox.MinY), tolerance);
            }
            else
            {
                // 按框选
        ...
        private void InitializeRenderer()
        {
            ...
            {
                sRender.Symbol = new moSimpleMarkerSymbol();
                _Renderer = sRender;
            }
            else if (_ShapeType == moGeometryTypeConstant.MultiPolyline)
            {
                sRender.Symbol = new moSimpleLineSymbol();
                _Renderer = sRender;
            }
            else
            {
                sRender.Symbol = new moSimpleFillSymbol();
                _Renderer = sRender;
            }
        }
        ...
        /// <param name="mapScale">比例尺</param>
        /// <param name="dpm"></param>
        /// <param name="mpu">一个坐标单位代表多少米</param>
        internal void DrawFeatures(Graphics g, moRectangle extent,
            double mapScale, double dpm, double mpu)
        {
            // (1) 为所有要素配置符号
            SetFeatureSymbols();
            // (2) 判断是否位于位置范围内，如是，则绘制
            Int32 sFeatureCount = _Features.Count;
            for(Int32 i = 0; i < sFeatureCount; ++i)
            {
                moFeature sFeature = _Features.GetItem(i);
        ...
        // 绘制指定范围内的所有选择要素
        internal void DrawSelectedFeatures(Graphics g, moRectangle extent,
            double mapScale, double dpm, double mpu, moSymbol symbol)
        {
            // 判断是否位于绘制范围内，如是，则绘制
        ...
        /// <param name="g"></param>
        /// <param name="extent"></param>
        /// <param name="mapScale">绘图比例尺</param>
        /// <param name="dpm">一米内的像素数</param>
        /// <param name="mpu">一个坐标单位等于多少米</param>
        /// <param name="placedLabelExtents">已经配置的所有注记在屏幕上的范围的集合</param>
        ...
        // 判断要素是否位于指定范围内，这里仅计算要素MBR和范围矩形是否相交
