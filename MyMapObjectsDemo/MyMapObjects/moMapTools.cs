...
namespace MyMapObjects
{
    public static class moMapTools
    {
        ...
        public static moPoint GetPedalPointOnLine(moPoint p,moPoints line)
        {
            moPoint sStart = line.GetItem(0);
            moPoint sEnd = line.GetItem(1);
            double sStart_P_X = p.X - sStart.X;
            double sStart_P_Y = p.Y - sStart.Y;
            double sStart_End_X = sEnd.X - sStart.X;
            double sStart_End_Y = sEnd.Y - sStart.Y;
            double sDot = sStart_P_X * sStart_End_X + sStart_P_Y * sStart_End_Y;
            double sLineLength = Math.Sqrt((sStart.X - sEnd.X) * (sStart.X - sEnd.X) + (sStart.Y - sEnd.Y) * (sStart.Y - sEnd.Y));
            double sRatio = sDot / sLineLength / sLineLength;

            double sPedal_X = sRatio * (sStart_End_X) + sStart.X;
            double sPedal_Y = sRatio * (sStart_End_Y) + sStart.Y;
            moPoint sPedal = new moPoint(sPedal_X, sPedal_Y);
            return sPedal;
        }
        #endregion
    }
